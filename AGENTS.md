...
- Element IDs are the coupling point between `index.astro` and `app.js` (`fileF`, `fileR`, `mottCutoff`, `qPhred`, `secPeak`, `primerF-file`/`primerF-text`, `primerR-*`, result `<pre>`s `strict`/`full`/`primerTrim`, `loadDemoBtn`, trace viewer: `traceSection`/`traceCanvasF`/`traceCanvasR`/`traceMeta`/`traceZoomIn`/`traceZoomOut`/`traceReset`/`traceAmp` (vertical, peak height)/`tracePos` (horizontal, X position). Renaming one side breaks the other silently.
